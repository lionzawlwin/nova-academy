...
void main() {
  group('secondaryCurriculumBank', () {
    test('contains exactly 104 modules', () {
      expect(secondaryCurriculumBank.length, 104);
    });
    ...
    test('has 12 modules per subject, except the coding/engineering '
        'STEAM-expansion subjects (4 each) and geography/history/computing '
      ...
      // module per grade (the "secondary depth batch 1" blocks), bringing
      // all three to 8; every other subject stays at 12.
    ...
    test('has 26 modules per grade', () {
      // Baseline 18 (6 subjects x 3 modules) plus 2 from the STEAM
      // expansion (1 coding + 1 engineering) plus 3 from the
      // history/geography/computing rollout (1 each) plus 3 more from the
      // "secondary geography/history/computing depth batch 1" blocks, per
      // grade.
      final counts = <String, int>{};
      for (final module in secondaryCurriculumBank) {
      ...
      for (final entry in counts.entries) {
        expect(
          entry.value,
          26,
          reason: '${entry.key} has ${entry.value} modules, expected 26',
        );
      }
    });

    test('has 3 modules per grade+subject combination, except every coding/'
        'engineering combo (1) and every geography/history/computing combo '
        '(2)', () {
      final counts = <String, int>{};
      for (final module in secondaryCurriculumBank) {
      ...
      const singleModuleSubjects = {'coding', 'engineering'};
