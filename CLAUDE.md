...
The Firebase project (`nova-acedemy`) is connected and live (see Deployment below); `flutter run` without a signed-in session falls back to signed-out/error states on screens touching Auth/Firestore rather than crashing.

## Architecture

**Routing (`lib/routing/app_router.dart`):** a single `goRouterProvider` with all redirect/RBAC logic centralized in one `_redirect()` function, driven by a `_RouterRefreshNotifier`. Route paths live in `AppRoutes` (string constants) — never hardcode path literals in screens. Do not add ad-hoc `Navigator.push`/imperative navigation outside this router; new routes/redirect rules belong in `_redirect()`.
...
**Grade → UI tier mapping:** When adding a new grade, update `Grade`, `homeTierForGrade()`, and the relevant ARB `grade*` keys together.
...
Firebase/Vercel/Android CI deployment details (targets, secrets, signing, seed-data mechanics): see the `deployment` skill (`.claude/skills/deployment/SKILL.md`), loaded on demand for deployment/CI/release-shaped tasks.

## Known Gaps (check before assuming a feature works end-to-end)

- Year 7-9 and IGCSE (Year 10-11) curriculum content is complete (`secondary_curriculum_bank.dart`, `course_pathways/`), alongside Nursery–Year 6 — full grade coverage Nursery through Year 11 exists across all curriculum banks. `Grade.year12` Physics (A-Level AS, Cambridge 9702) and Chemistry (A-Level AS, Cambridge 9701) are now fully authored — each all 9 terms / 36 weeks / 180 daily lessons (`course_pathways_physics.dart`/`year12PhysicsPathway`, `course_pathways_chemistry.dart`/`year12ChemistryPathway`). Every other Year 12 subject and all of `Grade.year13` are still placeholders only — no curriculum content is authored for them yet; they route to the Secondary/IGCSE dashboard shell and show an honest "not available" state.
