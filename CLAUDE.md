...
- **No Firebase project is connected.** There's no `firebase_options.dart` and `flutterfire configure` has never been run; `lib/main.dart` has no `Firebase.initializeApp()` call. Auth/Firestore calls throw `[core/no-app]` until this is wired up — the router and UI treat that as "signed out," not a crash, so the app is explorable but non-functional for real data until this is done.
- `firestore.rules` exists on disk but is undeployed.
...
- Seeding demo data (`seed_service.dart` — 1 parent, 2 teachers, 4 students "Alpha/Beta/Gamma/Delta" across all grade tiers, 7 bilingual learning modules) is triggered via a button on the Owner Admin screen (owner-only), not a standalone script — there's no Node/Admin SDK on this Spark/client-only stack. `tool/seed_database.dart` is a documentation stub only, not a runnable entry point.
